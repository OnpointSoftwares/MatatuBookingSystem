...
$eid=intval($_GET['eid']);
$status="0";
$sql = "UPDATE cyrus_testimonials_table SET status=:status WHERE  id=:eid";
...
$sql = "UPDATE cyrus_testimonials_table SET status=:status WHERE  id=:aeid";
$query = $dbh->prepare($sql);
$query -> bindParam(':status',$status, PDO::PARAM_STR);
						...
						<div class="panel panel-default">
							...
							<?php if($error){?><div class="errorWrap"><strong>ERROR</strong>:<?php echo htmlentities($error); ?> </div><?php } 
								...
								<table id="zctb" class="display table table-striped table-bordered table-hover" cellspacing="0" width="100%">
									...
									<?php $sql = "SELECT cyrus_users_table.FullName,cyrus_testimonials_table.UserEmail,cyrus_testimonials_table.Testimonial,cyrus_testimonials_table.PostingDate,cyrus_testimonials_table.status,cyrus_testimonials_table.id from cyrus_testimonials_table join cyrus_users_table on cyrus_users_table.Emailid=cyrus_testimonials_table.UserEmail";
$query = $dbh -> prepare($sql);
$query->execute();
